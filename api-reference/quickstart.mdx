...
```json
{
  ...
  "data": [
    { "id": 1972, "name": "Sucursal Centro", "isEnabled": true },
    { "id": 1973, "name": "Sucursal Norte", "isEnabled": true }
  ]
}
```

Guardá el `id` que corresponda en la configuración de cada caja. Es un dato que no cambia, así que alcanza con traerlo una vez.
...
<Tip>
  Los listados (`/external/products`, `/external/levels`, `/external/purchase`) devuelven `{ data, meta, links }` en la raíz, sin el envelope. Los premios están en `data`, no en `items`.
</Tip>
