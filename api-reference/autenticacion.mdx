...
| --- | --- | --- |
| No mandaste el header `x-api-key`, o vino vacío | `401` | `Api Key is required` |
| La key no corresponde a ningún comercio | `401` | `Api key is invalid` |

Los dos fallos de credencial son `401`. El `message` los separa: header ausente contra key que no pertenece a ningún comercio.

El `404` queda para cuando la key es válida y el recurso pedido no existe (`Client not found`, `Level not found`, `PointsItem with id 12 not found`).

## Probar que funciona

El endpoint más liviano para verificar que tu key está bien configurada es el de sucursales, contra QA (`https://apidev.tiendadepuntos.com`):
