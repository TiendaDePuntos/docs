...
<Note>
  Los endpoints de cupones son la excepción: en vez de este formato devuelven su propio objeto con `valid`, `consumed` y `error_code`. Está detallado en la ficha de cada uno.
</Note>
...
| --- | --- | --- |
| `400` | Datos inválidos, o una regla de negocio que no se cumple (canje ya entregado, código de validación incorrecto). | Revisá `errors` y `message`. No reintentar sin corregir. |
...
| `409` | Ya existe una operación con ese `external_id`. | **No reintentes**: los puntos ya se acreditaron. |
