...
| --- | --- | --- |
| `400` | Datos inválidos, o una regla de negocio que no se cumple (canje ya entregado, código de validación incorrecto, tope de operadores del plan alcanzado, `pointItems` vacío o ítems que no suman puntos). | Revisá `errors` y `message`. No reintentar sin corregir. |
| `401` | Falta `x-api-key` (`Api Key is required`), o está presente pero no corresponde a ningún comercio (`Api key is invalid`). | Revisá el header. No reintentes con la misma key. |
| `403` | El `branch_id` que mandaste no pertenece a tu comercio. | Verificá los IDs con `GET /external/branch`. |
| `404` | La key es válida y el recurso no existe (cliente, premio, nivel, ítem de puntos). | Mirá el `message`. |
...
<Note>
  Sin header, o con el header vacío, la respuesta es `401` y `Api Key is required`. Con una key que no corresponde a ningún comercio, también es `401`, con `Api key is invalid`. El `404` queda para un recurso que no existe en tu comercio (`Level not found`, `Client not found`, `PointsItem with id 12 not found`).
</Note>
...
Cada límite se cuenta de forma distinta, según la columna de la tabla.

| Endpoints | Límite |
| --- | --- |
| `/external/tags/*` | 10 requests por segundo, por IP de origen |
| `POST /external/tags/add/preview` | 30 requests por segundo, por IP de origen. Es un cupo aparte: simular no consume el de la suma real |
