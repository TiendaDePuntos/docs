...
| `401` | El header `x-api-key` está presente pero no corresponde a ningún comercio (`Api key is invalid`). | Revisá la key. No reintentes con la misma. |
| `403` | El `branch_id` que mandaste no pertenece a tu comercio. | Verificá los IDs con `GET /external/branch`. |
| `404` | Falta el header `x-api-key` (`Api Key is required`), **o** el recurso no existe. | Mirá el `message` para distinguir los casos. |
...
<Note>
  Una key presente pero inválida responde `401` con `Api key is invalid`. El `404` cubre otros dos casos: falta el header `x-api-key` (`Api Key is required`) y el recurso inexistente (`Level not found`, `Client not found`, `Sucursal no encontrada`).
</Note>
